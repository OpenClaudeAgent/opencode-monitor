...
# =============================================================================
# Singleton Cleanup for pytest-xdist (prevents cross-test pollution)
# =============================================================================


def _cleanup_all_singletons():
    """Reset all module-level singleton instances (12 total)."""
    # 1. Security Auditor
    try:
        from opencode_monitor.security.auditor import core as auditor_core

        if auditor_core._auditor is not None:
            try:
                auditor_core._auditor.stop()
            except Exception:
                pass
    ...
    try:
        ...
        if (
            hasattr(server_module, "_api_server")
            and server_module._api_server is not None
        ):
            try:
                server_module._api_server.stop()
            except Exception:
                pass
    ...
    try:
        ...
        if hasattr(client_module, "_api_client"):
    ...
    try:
        ...
        if hasattr(risk_module, "_analyzer"):
    ...
    try:
        ...
        if (
            hasattr(cache_module, "_thumbnail_cache")
            and cache_module._thumbnail_cache is not None
        ):
            try:
                cache_module._thumbnail_cache._cache.clear()
            except Exception:
                pass
    ...
    # 10. Strategy Factory
    try:
        import opencode_monitor.dashboard.sections.tracing.detail_panel.strategies as strat_module

        if hasattr(strat_module, "_factory_instance"):
            strat_module._factory_instance = None
    except ImportError:
        pass
    ...
    try:
        ...
        if hasattr(launcher_module, "_dashboard_process"):
...
def reset_singletons_per_worker(request):
    """Reset singletons at worker startup/shutdown for pytest-xdist isolation."""
